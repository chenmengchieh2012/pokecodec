...
export class MigrationManager {
    ...
    public async checkAndPerformMigrations() {
        ...
        const isFirstRunCheck = this.context.globalState.get('isFirstRun', true);
        const hasMigratedTo002 = this.context.globalState.get<boolean>('hasMigratedTo002', false);

        if (!isFirstRunCheck && !hasMigratedTo002) {
            console.log('[Extension] Performing migration to version 0.0.2 (Economy Reset)');
            
            // 1. 重設金錢為 5000
            const currentMoney = this.userDaoManager.resetMoney(5000);
            ...
            // 標記遷移已完成
            await this.context.globalState.update('hasMigratedTo002', true);
        }
        
        // 更新當前版本號
        const currentVersion = this.context.extension.packageJSON.version;
        await this.context.globalState.update('extensionVersion', currentVersion);
    }
}
